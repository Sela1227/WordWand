...
> 介面為兒童活潑風格、全程繁體中文、零 emoji。本專案經 SELA 指示**不套用 SELA 品牌 logo**,使用專屬的可愛標記。
...
1. 在 Railway 新建專案,從 GitHub repo 部署,**root directory 設為 `backend/`**
...
把 `backend/main.py` 的 CORS `allow_origins=["*"]` 改成你的 GitHub Pages 網址,只允許自己的網站呼叫後端。
...
├── backend/              Railway 後端(root directory 設為 backend)
│   ├── main.py           FastAPI 代理(藏 key + 兒童安全把關)
