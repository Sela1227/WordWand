...
> 設計原則(訓練作文能力):優先做「教學/鷹架型」功能(引導、提示、講原因),少做「代寫型」功能,否則只是給答案、訓練不到能力,也可能變成代寫工具。

1. **句子醫生(self-revision)** — 第 1 名:不只給漂亮版本,還指出原句哪裡弱(流水帳/用詞重複/太短)+ 怎麼改,最能真正練到能力。
2. **段落擴寫鷹架** — 小朋友寫一句,精靈用引導問題(然後呢?當時心情?看到什麼?)幫他擴成一段,不直接代寫。
3. **點子產生器 / 主題聯想** — 給主題詞,精靈丟感官提示幫發想(寫作最難是「沒得寫」)。
4. **修辭魔法** — 教譬喻/擬人/排比,把白句加上一個比喻並解釋。
5. **作文題目 + 大綱引導** — 給適齡題目,協助列開頭-經過-結尾三段大綱(練結構)。
6. 結果加「複製給老師看」按鈕(一鍵複製純文字)。
7. 成語寶庫(集點):把學過的成語收集起來,localStorage 存(GitHub Pages 可用),練動機。
8. 把精靈人格、模式、SAFETY 抽成 `config.json`;速率限制未來升跨 replica 版。
...
V0.3.1:手機介面優化(點擊延遲、安全區、點按回饋、觸控範圍);至此功能完整且上線(後端 Railway + 前端 Pages,四層把關齊全)。下版方向轉向「訓練作文能力」的教學型功能,第一優先是『句子醫生』(指出原句弱點 + 教怎麼改,而非只給漂亮版本)。
